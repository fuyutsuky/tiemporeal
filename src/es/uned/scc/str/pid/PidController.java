 ...
 * 
 * @see http
 *      ://brettbeauregard.com/blog/2011/04/improving-the-beginners-pid-direction
...
public class PidController {
	...
	/**
	 * Create a PidController class object. See setP, setI, setD methods for
	...
	/**
	 * Create a PidController class object. See setP, setI, setD, setF methods
	 ...
	 * "Position" based modes, and third for "Rate" or continuous based modes. <br>
	 ...
	 * This has two primary effects: <list> <li>Adds a "startup kick" and speeds
	 * up system response during setpoint changes <li>Adds "drag" and slows the
	 * system when moving toward the target </list> A small D value can be
	 * useful for both improving response times, and preventing overshoot.
	...
	public double getOutput(double actual, double setpoint) {
		...
		if (setpointRange != 0) {
			setpoint = constrain(setpoint, actual - setpointRange, actual
					+ setpointRange);
		}
		...
		if (minOutput != maxOutput && !bounded(output, minOutput, maxOutput)) {
			...
			// From that point the I term will build up as would be expected
				...
				&& !bounded(output, lastOutput - outputRampRate, lastOutput
						+ outputRampRate)) {
			errorSum = error;
		...
		if (outputRampRate != 0) {
			output = constrain(output, lastOutput - outputRampRate, lastOutput
					+ outputRampRate);
		}
